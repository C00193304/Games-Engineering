...
Particles::Particles(b2World* pWorld, Cannon* cannon, int amount)
{
	...
	ParentsParticles.bodyDef.position.Set(cannon->GetRect().pos.x, cannon->GetRect().pos.y);
...
void Particles::Init(b2World* pWorld, int amount)
{
	if (amount + particles.size() > MAXIMUM_PARTICLES)
	{
		amount = MAXIMUM_PARTICLES - particles.size();
...
void Particles::Update(unsigned int deltaTime)
{
	if (particleAmount >= 0)
	{
		amountParticles(1);
...
void Particles::Render(Renderer & r)
{
	for (int i = 0; i < particles.size(); i++)
	{
...
void Particles::amountParticles(int amount)
{
	/*if (amount + particles.size() > MAXIMUM_PARTICLES)
	{
		amount = MAXIMUM_PARTICLES - particles.size();
