...
void Level6::init()
{
	...
	// Create obstacles
	Box* test = new Box(Rect(-100, 500, 300, 100));
...
void Level6::Draw(Renderer& renderer)
{
...
void Level6::Update(unsigned int DeltaTime)
{
	Rect tempRect;
	...
	for (std::vector<GameObject*>::iterator i = GameObjects.begin(), e = GameObjects.end(); i != e; i++)
	{
		...
		if ((*i)->GetType() == "Gate")
		{
			...
			{
				init();
				SceneManager::ChangeStates(SceneManager::end);
				//reset this scene.
				//go to next scene.
				break;
			}
...
void Level6::onEvent(EventListener::Event evt)
{
	switch (evt) {
...
bool Level6::CheckCenter(Rect player, Rect gate)
{
