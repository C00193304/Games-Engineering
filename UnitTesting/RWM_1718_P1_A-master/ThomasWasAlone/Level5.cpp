...
{
	// Create Player
	player = new Player(Rect(100, 460, 20, 39));
	player->SetColour(Colour(255, 162, 76));

	GameObjects.clear();

	// Create obstacles
	Box* test = new Box(Rect(0, 500, 200, 100));
	test->SetColour(Colour(0, 0, 0, 255));
	GameObjects.push_back(test);

	test = new Box(Rect(300, 500, 200, 100));
	test->SetColour(Colour(0, 0, 0, 255));
	GameObjects.push_back(test);

	test = new Box(Rect(600, 500, 200, 100));
	test->SetColour(Colour(0, 0, 0, 255));
	GameObjects.push_back(test);
	...
	Gate* gate = new Gate(Rect(750, 460, 20, 40));

	GameObjects.push_back(gate);
}

void Level5::Draw(Renderer& renderer)
{
...
void Level5::Update(unsigned int DeltaTime)
{
	Rect tempRect;
	...
	for (std::vector<GameObject*>::iterator i = GameObjects.begin(), e = GameObjects.end(); i != e; i++)
	{
		...
		if ((*i)->GetType() == "Gate")
		{
			...
			{
				init();
				SceneManager::ChangeStates(SceneManager::level6);
				//reset this scene.
				//go to next scene.
				break;
			}
...
void Level5::onEvent(EventListener::Event evt)
{
	switch (evt) {
...
bool Level5::CheckCenter(Rect player, Rect gate)
{
